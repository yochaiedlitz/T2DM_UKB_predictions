...
def Calc_Final_Proba(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, X, y, X_test, y_test, n_folds,
    ...
    run_name = str(pd.read_csv(os.path.join(final_folder,job_name + "_result_sorted.csv"), index_col=0).index.values[0])
    ...
    if calc_shap:
        shap_values_df.to_csv(os.path.join(final_folder,job_name + "_shap_values.csv"), index=False)
...
def create_pdf_prob(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, n_folds, y_test, X_test,
    ...
    if calc_shap:
        ...
        df = pd.read_csv(os.path.join(final_folder, job_name + "_shap_values.csv"))
        ...
        plt.savefig(os.path.join(final_folder, "Results_" + job_name + "_" + run_name + 'SHAP.png'), bbox_inches='tight', dpi=DPI)
        plt.close("all")
        img = mpimg.imread(os.path.join(final_folder ,"Results_" + job_name + "_" + run_name + 'SHAP.png'))
        imgplot = plt.imshow(img)
        plt.axis('off')
