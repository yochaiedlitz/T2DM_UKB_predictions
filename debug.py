...
def create_pdf(Save_2_folder, final_folder, job_name, Target_ID, proba_path, calc_shap, mode, use_proba, pdf_name,
    ...
    with open(final_folder + job_name + "train_Data", 'rb') as fp:
        Data = pickle.load(fp)
...
def Calc_SHAP(final_folder, pdf, job_name, gbm):
    ...
    for name in Ordered_X_idx[0:NUM_OF_DEP_PLOT]:
        ...
        plt.savefig(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png', bbox_inches='tight',
                    dpi=SHAP_DPI)
        plt.close("all")
        img = mpimg.imread(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png')
        imgplot = plt.imshow(img)
...
def Load_N_Data(data_path, Target_ID, final_folder, Sub_Class_ID, job_name, feat_path, no_symp_code, mode, use_proba,
                ...
                Thresh_in_Row=0.7, split=True, train_set=True):
    """
    ...
    if FEAT_DF.shape[0] != 0:
        # cat_names = [Feat_DF_eid.loc[y, "Description"]+x.split("_")[-1] for x in cat_feat for y
        #              in Feat_DF_eid if y.startswith(x.split("_")[0])]
    ...
    if use_proba:
        ...
        Data = {'df_Features': df_Features, 'DF_Targets': DF_Targets, 'X_display': X_display, 'X_train': X_train,
                ...
                'Rel_Feat_Names': Rel_Feat_Names}
    if train_set == True:
        with open(final_folder + job_name + "train_Data", 'wb') as fp:
            pickle.dump(Data, fp)
    else:
        with open(final_folder + job_name + "test_Data", 'wb') as fp:
            pickle.dump(Data, fp)
    return Data
...
def Sort_Prob_AUC_APS(job_name, Save_2_folder, final_folder, Target_ID, calc_shap, use_proba, X, y, X_test, y_test,
                      metric, n_folds,
                      pdf_name):
    print()
    "pdf name in Sort_Prob_AUC_APS is:", pdf_name
    ...
    if os.path.isfile(final_folder + job_name + "_Parameters_Table.csv"):
        ...
        print()
        "in Sort_Prob_AUC_APS, path:", path
        print()
        "Sort_AUC_APS concatanation path:", path
        allFiles = glob.glob(path)
        print()
        "allFiles:", allFiles
        frame = pd.DataFrame()
...
def Calc_Final_Proba(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, X, y, X_test, y_test, n_folds,
    ...
    if Refit_Return_Model_Path == None:
        ...
        #             valid_names=["Train"], feature_name=Rel_Feat_Names,
        #             evals_result=evals_result, verbose_eval=VERBOSE_EVAL)
    gbm.save_model(final_folder + "Results_" + job_name + "_" + run_name + "_lgbm.txt")
    ...
    if not Calc_only_shap:
        ...
        gbm.save_model(final_folder + job_name + run_name + '_lgbm.txt')
    ...
    if ~Calc_only_shap:
        with open(final_folder + job_name + "_evals_result", 'wb') as fp:
            pickle.dump(evals_result, fp)
        with open(final_folder + job_name + "y_proba", 'wb') as fp:
            pickle.dump(y_proba, fp)
        y_proba_df.to_csv(final_folder + job_name + "_OnlyPROB.csv")
...
def create_pdf_prob(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, n_folds, y_test, X_test,
    ...
    run_name = str(
        pd.read_csv(filepath_or_buffer=final_folder + job_name + "_result_sorted.csv", index_col=0).iloc[0, 0])
    SHAP_PDF_PATH = final_folder + pdf_name + "_" + str(run_name) + ".pdf"
    pdf = PdfPages(SHAP_PDF_PATH)
    y_pred_val = []
    y_test_val = []
