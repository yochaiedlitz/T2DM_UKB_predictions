...
class ScoreBoard:
    def __init__(self, scoreboard_type,
                 ...
                 recover=False,
                 min_num_of_ills=1):  # Anthro,Five_blood
        """
        ...
        self.leg_dict = {"LR_Five_blood_tests_scoreboard": "Five blood tests ScoreBoard",
        ...
        self.run_object = runs(self.run_name, mode=self.mode,debug=self.debug)
        ...
        self.tables_save_path = "/net/mraid08/export/jafar/Yochai/UKBB_Runs/For_article/Imputed_screened/Tables/"
        self.features_importance_tables_save_path=os.path.join(self.tables_save_path,self.run_name+
                                                        "_importance.csv")
        self.ci_summary_table_path = self.run_name + "_features_importance_summary.csv"
    ...
    def bin_dataframes(self):
        ...
        self.train_bins_df, self.train_bins_df_dict, self.train_bins_labels_dict,self.woe_res_dict = bin_df(
            self.X_train, self.y_train, self.features_file_df, max_bins=5, res_dict=self.woe_res_dict,min_num_of_ills=self.min_num_of_ills)
    ...
    def calculate_scoreboards_bins_scores(self,
        ...
        try:
            ...
            lr_features_coeff_obj = LR_Feature_importance(
                ...
                remove_legend=True,
                ci_summary_table_name=self.ci_summary_table_path, build_new=build_new_feature_importance
            )
...
def recover_scoreboard(scoreboard_type):
...
def find_bin(row, bin_limits):
    for ind, val in enumerate(bin_limits):
        if ind < (len(bin_limits) - 1) and (row <= bin_limits[ind + 1]):
            ...
            return ind
    sys.exit("No bin was found for val:", val)
...
def concat_train_val_test_dataframes(Orig_df_file_path, scoretable_df, save_to_path):
    ...
    New_df = pd.concat([Orig_df, scoretable_df.loc[:, new_columns]], axis=1)
