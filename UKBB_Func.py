...
SAVE_FOLDER = "/net/mraid08/export/jafar/Yochai/UKBB_Runs/For_article/"
...
def Sort_AUC_APS(job_name, Save_2_folder, final_folder, Target_ID, proba_path, calc_shap, mode, use_proba, pdf_name,
    ...
    path = Save_2_folder+'*_result.csv'
    ...
    df.to_csv(final_folder+job_name+"_Result.csv")

    auc_aps = df
# Meging the parameters file

    path = Save_2_folder+'*parameters.csv'
    ...
    frame.to_csv(final_folder + job_name + "_Parameters_Table.csv")
...
def print_to_file(result, job_name, final_folder, Save_2_folder,  frame, Target_ID
    ...
    result_sorted.to_csv(final_folder+job_name + "_result_sorted.csv")
    top_auc_sn = str(int(result_sorted.index.values[0]))
    create_pdf(Save_2_folder, final_folder, job_name,  Target_ID, proba_path, calc_shap, mode, use_proba, pdf_name,Refit_Base_Model_Path)


def create_pdf(Save_2_folder, final_folder, job_name, Target_ID,proba_path, calc_shap, mode, use_proba, pdf_name, Refit_Base_Model_Path):
    ...
    with open(final_folder + job_name + "_Data", 'rb') as fp:
        Data = pickle.load(fp)
    ...
    run_name = str(pd.read_csv(filepath_or_buffer=final_folder+job_name + "_result_sorted.csv").iloc[0,0])

    # ~~~~~~~~~~~~Loading parameters~~~~~~~~~~~~~~
    parameters = pd.read_csv(final_folder + job_name + "_Parameters_Table.csv", index_col=0) # Check that we can read params and build the selected model, train it and make all required drawings
    ...
    SHAP_PDF_PATH = final_folder+pdf_name+"_"+run_name + ".pdf"
    ...
    if use_proba:
        if Refit_Base_Model_Path == None:
            gbm = lgb.train(init_model=Refit_Base_Model_Path, params=params_dict ,num_boost_round=num_trees,
                        ...
                        evals_result=evals_result, verbose_eval=VERBOSE_EVAL,num_threads=10)
        else:
            Base_gbm = lgb.Booster(model_file=Refit_Base_Model_Path)
            gbm = lgb.train(params=params_dict ,num_boost_round=Base_gbm.num_trees()+num_trees, train_set=lgb_train, valid_sets=[lgb_train, lgb_eval],
                        valid_names=["Train", "Evaluation"], feature_name=Rel_Feat_Names,
                        evals_result=evals_result, verbose_eval=VERBOSE_EVAL,num_threads=10)  #
    else:
        if Refit_Base_Model_Path == None:
            gbm = lgb.train(init_model=Refit_Base_Model_Path, params=params_dict,num_boost_round=num_trees, train_set=lgb_train, valid_sets=[lgb_train],
                        valid_names=["Train"], feature_name=Rel_Feat_Names,
                        evals_result=evals_result, verbose_eval=VERBOSE_EVAL,num_threads=10)

        else:
            Base_gbm = lgb.Booster(model_file=Refit_Base_Model_Path)
            gbm = lgb.train(params=params_dict,num_boost_round=Base_gbm.num_trees()+num_trees, train_set=lgb_train, valid_sets=[lgb_train],
                        valid_names=["Train"], feature_name=Rel_Feat_Names,
                        evals_result=evals_result, verbose_eval=VERBOSE_EVAL,num_threads=10)  #

    print("Final training of ", job_name, " took %s seconds ---" % (time.time() - start_time))
    gbm.save_model(final_folder + run_name + '_lgbm.txt')

    with open(final_folder + "evals_result", 'wb') as fp:
    ...
    y_proba = gbm.predict(X_test, num_iteration=num_trees, raw_score=False,num_threads=10)
    ...
    y_proba_df.to_csv(final_folder + job_name+"_OnlyPROB.csv")

    if ((mode == "A") and (use_proba)):
        X_test[job_name+"Y_PROB"] = y_proba
        X_test.to_csv(final_folder + job_name + "_FEATandY_PROB.csv")
        y_test.to_csv(final_folder + job_name + "_Target.csv")
        with open(final_folder + job_name + "_Proba", 'wb') as fp:
            pickle.dump(y_proba, fp)
        with open(final_folder + job_name + "_Feat", 'wb') as fp:
    ...
    if Target_ID != "21001-0.0":
        ...
        plot_calibration_curve(y_test_val, y_pred_val, pdf, final_folder + job_name, Print_Benefit=True)

    with open(final_folder + "y_pred_val_list", 'wb') as fp:
        pickle.dump(y_pred_val, fp)
    with open(final_folder + "y_test_val_list", 'wb') as fp:
        pickle.dump(y_test_val, fp)

    X_display = X_test.copy()
    X_display.columns = Rel_Feat_Names
    if ~os.path.isfile(final_folder + job_name + "_X_display.csv"):
...
def plot_only(final_folder, calc_shap, job_name,gbm=None):
    ...
    SHAP_PDF_PATH = final_folder+"Results_"+job_name+"_"+run_name + ".pdf"
    pdf = PdfPages(SHAP_PDF_PATH)
    with open(final_folder + job_name + "_Proba", 'rb') as fp:
        y_pred_val = pickle.load(fp)
    with open(final_folder + "y_test_val_list", 'rb') as fp:
...
def Calc_SHAP(final_folder, pdf, job_name, gbm):
    ...
    SHAP_PDF_PATH = final_folder + "Results_" + job_name + "_" + run_name + ".pdf"
    # ~~~~~~~~~~~~Loading parameters~~~~~~~~~~~~~~
    parameters = pd.read_csv(final_folder + job_name + "_Parameters_Table.csv",
    ...
    gbm.save_model(final_folder+job_name+"_shap_model.txt")
    df = pd.DataFrame(shap_values)
    df.to_csv(final_folder+job_name+"_shap_values.csv")
    # X_display.to_csv(final_folder+JOB_NAME+"_X_display.csv")

    fig = plt.figure()
    with open(final_folder + "Rel_Feat_Names", 'rb') as fp:
        Rel_Feat_Names = pickle.load(fp)
    X_display = X_test.copy()
    X_display.columns = Rel_Feat_Names
    if shap_values.shape[0]>50000:
        shap.summary_plot(shap_values[:50000,:X_display.shape[1]], features=X_display.iloc[:50000,:], show=False, auto_size_plot=True,
                          max_display=30)
    else:
        shap.summary_plot(shap_values[:,:X_display.shape[1]], features=X_display, show=False, auto_size_plot=True,
                          max_display=30)
    plt.savefig(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png',bbox_inches='tight',dpi=SHAP_DPI)
    plt.close("all")
    img = mpimg.imread(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png')
    ...
    if FEAT_DF.shape[0] != 0:
        ...
        Sorted_Question.to_csv(final_folder + job_name + "_Features_List_Shap_Sorted.csv")
        ...
        for ind, quant in enumerate(Quants.values):
            if ind > 0:
                ...
                Rank.loc[np.str(ind), "All"] = df.loc[((df["Y_Pred"] <= quant) & \
                                                       (df["Y_Pred"] > Quants.values[ind - 1]))].loc[:, 'Y_test'].count()
...
def Filter_CZ(Features_DF, charac_selected, charac_id):  # CHARAC_SELECTED = {"Age at recruitment": "All", "Sex": "Female", "Ethnic background":
    ...
    # CHARAC_ID = {"Age at recruitment": 21022, "Sex": 31, "Ethnic background": 21000,
    # "Type of special diet followed": 20086}

    if charac_selected["Age at recruitment"] != "All":
        Age_max = charac_selected["Age at recruitment"]+2.5
        Age_min = charac_selected["Age at recruitment"]-2.5
        Features_DF = Features_DF.loc[Features_DF[charac_id["Age at recruitment"]].between(Age_min, Age_max, inclusive=True),:]
...
def Load_Targets(Target_ID, Sub_Class_ID, visits, nrows_return, no_symp_codes,data_path=DATA_PATH):
    # if debug:
    #     fname = SAVE_FOLDER+"debug/"+Target_ID+"Re_all_Targets.csv"
    # else:
    #     fname = SAVE_FOLDER+"Data_Files/"+Target_ID+"Re_all_Targets.csv"
    DF_Targets = []
    ...
    if Target_ID != 21001-0.0:
        DF_Targets[0] = DF_Targets[0][DF_Targets[0].iloc[:, 0] == 0] #Taking only Health persons in round A that flipped in round B
    ...
    if len(DF_Targets) >= 2:
        Targets = pd.concat([DF_Targets[1], DF_Targets[2]],axis=0,join="outer").groupby(level=0).max() #if  participant visits twice, taking the value if he got sick
...
def Load_N_Data(Target_ID, final_folder, Sub_Class_ID, job_name, feat_path, no_symp_code, mode, use_proba,
    ...
    sep = "-"
    Non_Used_Columns = [x for x in Use_Columns if x not in data_cols.columns.values]
    ...
    if FEAT_DF.shape[0] != 0:
        ...
        FEAT_DF[FEAT_DF["Field ID"].isin(Non_Used_Columns)].to_csv(final_folder + job_name + "_non_used_col.csv")
    ...
    cat_feat = []
    if FEAT_DF.shape[0] != 0:
        temp_Feat = FEAT_DF.iloc[:, [0, 3]]
        cat_feat = FEAT_DF.loc[
            (FEAT_DF['Datatype'] == 'Categorical') & (FEAT_DF["Exclude"] != 1), "Field ID"].values #All categorical Features
    # cat_feat = [str(int(s)) + "-0.0" for s in cat_feat]
    cat_feat = [s for s in cat_feat if s in filtered_cols]  # Leaving only features that exist in our database
    # if FEAT_DF.shape[0] != 0:
    #     cat_feat_names=[Feat_DF_eid.loc[np.int64(s[:-4]), "Description"] for s in cat_feat if np.int64(s[:-4]) in Feat_DF_eid.index]
    # else:
    #     cat_feat_names=[]
    # Rel_cat_Feat = [k[:-4] for k in cat_feat if k[:-4] != ""]
    # Rel_cat_Feat = [int(k) for k in Rel_cat_Feat]
    if FEAT_DF.shape[0] != 0:
        cat_names = FEAT_DF.loc[FEAT_DF['Field ID'].isin(cat_feat), "Description"].values
    else:
        cat_names=[]

    # FEAT_DF.to_csv()
    # Loading data from UKB
    ...
    # df_Features.dropna(axis="columns", thresh=(int(Thresh_in_Column * df_Features.shape[0])),inplace=True)
    # df_Features.dropna(axis="index",thresh=(int(Thresh_in_Row*df_Features.shape[1])),inplace=True)
    # print 'df_Features size after filtering nan', df_Features.shape
    ...
    if df_Features.shape[0] != 0:
        ...
        # Rel_Feat.extend(["21003-1.0","21003-2.0"])
        # Rel_Feat_Names.extend(["Age at first assessment center return","Age at second assessment center return"])
    if use_prs: #Adding PRS -Only final score for each phenotype for each user
        ...
        PRS_DF = pd.read_csv(filepath_or_buffer=prs_path, index_col="eid", usecols=prs_use_col.append("eid"))
        if df_Features.shape[0]!=0:
            how_merge = "inner"
        else:
            how_merge = "outer"
    ...
    if split:
        ...
        if ((mode == "A") and use_proba):
            ...
            y_test = Returning_targets.loc[test_index].reset_index().drop_duplicates(subset='eid', keep='first').set_index("eid")
            y_train = DF_Targets.loc[train_index, :].reset_index().drop_duplicates(subset='eid', keep='first').set_index("eid")
            X_test = df_Features.loc[test_index, :].reset_index().drop_duplicates(subset='eid', keep='first').set_index("eid")
            X_train = df_Features.loc[train_index, :].reset_index().drop_duplicates(subset='eid', keep='first').set_index("eid")
            ...
            with open(final_folder + "y_val", 'wb') as fp:
                pickle.dump(y_test, fp)
            y_val.to_csv(final_folder + "y_val.csv")

            with open(final_folder + "X_val", 'wb') as fp:
                pickle.dump(X_test, fp)
            X_val.to_csv(final_folder + "X_val.csv")
        ...
        with open(final_folder + "y_test", 'wb') as fp:
            pickle.dump(y_test, fp)
        y_test.to_csv(final_folder + "y_test.csv")

        with open(final_folder + "X_test", 'wb') as fp:
            pickle.dump(X_test, fp)
        X_test.to_csv(final_folder + "X_test.csv")

        with open(final_folder + "y_train", 'wb') as fp:
            pickle.dump(y_train, fp)
        y_train.to_csv(final_folder + "y_train.csv")

        with open(final_folder + "X_train", 'wb') as fp:
            pickle.dump(X_train, fp)
        X_train.to_csv(final_folder + "X_train.csv")

        with open(final_folder + "cat_names", 'wb') as fp:
            pickle.dump(cat_names, fp)
        with open(final_folder + "Rel_Feat_Names", 'wb') as fp:
            pickle.dump(Rel_Feat_Names, fp)

        if ~os.path.isfile(final_folder + job_name + "_X_display.csv"):
            X_display.to_csv(final_folder + job_name + "_X_display.csv")
        if ~os.path.isfile(final_folder + job_name + "_df_Features.csv"):
            df_Features.to_csv(final_folder + job_name + "_df_Features.csv")
        if ~os.path.isfile(final_folder + job_name + "_DF_Targets.csv"):
            DF_Targets.to_csv(final_folder + job_name + "_DF_Targets.csv")
        if ~os.path.isfile(final_folder + job_name + "_DF_Features_List.csv"):
            FEAT_DF.to_csv(final_folder + job_name + "_DF_Features_List.csv")

        if use_proba:
            Data = {'df_Features': df_Features, 'DF_Targets': DF_Targets, 'X_display': X_display, 'X_train': X_train,
                    'y_train': y_train, 'y_test': y_test, 'X_test': X_test, 'cat_names': cat_names,'y_val': y_val, 'X_val': X_val,
                    'Rel_Feat_Names': Rel_Feat_Names}
        else:
            Data = {'df_Features': df_Features, 'DF_Targets': DF_Targets, 'X_display': X_display, 'X_train': X_train,
                    'y_train': y_train, 'y_test': y_test, 'X_test': X_test, 'cat_names': cat_names,
                    'Rel_Feat_Names': Rel_Feat_Names}
    else: #Mostly used for DNN where splitting done in batches
        X_display = df_Features.copy()
        X_display.columns = Rel_Feat_Names
        Data = {'df_Features': df_Features, 'DF_Targets': DF_Targets, 'X_display': X_display, 'cat_names': cat_names,
                'Rel_Feat_Names': Rel_Feat_Names}
    with open(final_folder + job_name + "_Data", 'wb') as fp:
        pickle.dump(Data, fp)
    return Data
...
def Predict(SN, parameters, Save_2_folder, job_name, final_folder, n_fold,use_proba=True,Refit_Base_Model_Path=None):
    ...
    print(("---Start training non-shap values of  model : ", str("SN"), ", at Predict, at time: %s ---" % datetime.datetime.now()))
    ...
    print("parameters before train:", parameters)
    num_trees=int(parameters['num_boost_round'])
    if Refit_Base_Model_Path == None:
        gbm = lgb.train(params=parameters, num_boost_round=num_trees,train_set=lgb_train, valid_sets=[lgb_train, lgb_test], valid_names=["Train", "Valid"],
                        feature_name=Rel_Feat_Names, evals_result=evals_result, verbose_eval=VERBOSE_EVAL)  #
    else:
        Base_gbm=lgb.Booster(model_file=Refit_Base_Model_Path)
        print("Num of trees before refit:", str(Base_gbm.num_trees()))
        gbm = lgb.train(init_model=Refit_Base_Model_Path, params=parameters, num_boost_round=Base_gbm.num_trees()+num_trees,
                            train_set=lgb_train, valid_sets=[lgb_train, lgb_test], valid_names=["Train", "Valid"],
                            feature_name=Rel_Feat_Names, evals_result=evals_result, verbose_eval=VERBOSE_EVAL)
        print("Num of trees After refit:", str(gbm.num_trees()))

    gbm.save_model(final_folder + job_name + "_CV_Model_" + run_name +".txt")
    ...
    cv_pd.to_csv(Save_2_folder + run_name + '_cv_result.csv')

    params_df.loc["SN"] = SN
    print("Save params_df in predict to: ", Save_2_folder, run_name,"_parameters.csv")
    params_df.to_csv(Save_2_folder + run_name + "_parameters.csv")
...
def Predict_prob(SN, parameters, X, y, cat_names, Rel_Feat_Names, Save_2_folder, job_name, n_fold, final_folder,
                 Refit_Return_Model_Path,Refit_Returned):
    run_name = str(SN)
    print("Rel_Feat_Names:", Rel_Feat_Names)
    print("started predict prob of SN:", run_name)
    path = Save_2_folder + "SN" + run_name + "_parameters.csv"
    start_time = time.time()
    cv_ind = 0
    Test_ind_list = []
    Train_ind_list=[]
    ...
    for train_index, test_index in skf.split(X, y.values.flatten()):
        ...
        if not Refit_Returned: #Case whithut refit
            ...
            cv_result = lgb.cv(init_model=sort_lgbs[0],params=parameters, num_boost_round=num_of_trees, train_set=lgb_train,
                               feature_name=Rel_Feat_Names, nfold=n_fold, stratified=True)
        else: #Refitting model calculated before on same data (stacking)
            cv_result = lgb.cv(init_model=Refit_Return_Model_Path+sort_lgbs[cv_ind],params=parameters, num_boost_round=num_of_trees, train_set=lgb_train,
                               feature_name=Rel_Feat_Names, nfold=n_fold, stratified=True)
        ...
        cv_pd.to_csv(Save_2_folder + run_name + '_cv_ind' + str(cv_ind) + '_cv_result.csv')
        if cv_ind == 0:
        ...
        cv_ind += 1
        print(("Cross validation of cv_ind:", str(cv_ind)," of SN: ", str(SN), "took %s seconds ---" % (time.time() - start_time)))

    # params_df_Total.to_csv(path)
    with open(final_folder + "Test_ind_list_" + str(SN), 'wb') as fp:
        pickle.dump(Test_ind_list, fp)
    print("finished saving Test_ind_list of:", str(cv_ind))

    with open(final_folder + "Train_ind_list" + str(SN), 'wb') as fp:
        pickle.dump(Train_ind_list, fp)
    print("finished saving Train_ind_list of:", str(cv_ind))


def AVG_Prob_AUC_APS_per_SN (job_name, Save_2_folder, final_folder,  SN):

    path = Save_2_folder + str(SN)+'_cv_ind*_cv_result.csv'
    files = glob.glob(path)
    scores = []
    print("in AVG_Prob_AUC_APS_per_SN files:", files)
    for name in files:  # 'file' is a builtin type, 'name' is a less-ambiguous variable name.
        scores.append(pd.read_csv(name).iloc[-1:, -3:])
    df = pd.concat(scores)
    ...
    fname = final_folder + job_name + "_Score_Table.csv"
    ...
    print("Scores Data frame at the end of AVG_Prob_AUC_APS_per_SN:", avg_score_df)

def Sort_Prob_AUC_APS(job_name, Save_2_folder, final_folder, Target_ID, calc_shap, use_proba,X, y, metric, n_folds,
                      pdf_name):
    ...
    if os.path.isfile(final_folder + job_name + "_Parameters_Table.csv"):
        frame = pd.read_csv(final_folder + job_name + "_Parameters_Table.csv")
        frame.set_index(frame.columns[0], drop=True,inplace=True)
        frame.index.name = "parameter"

    else:
        path =Save_2_folder + "SN*_parameters.csv"
    ...
    Sort_Prob(auc_aps, job_name, final_folder, Save_2_folder, frame, Target_ID, calc_shap,  X, y, n_folds,pdf_name)


def Sort_Prob(result, job_name, final_folder, Save_2_folder, frame, Target_ID, calc_shap, X, y, n_folds,pdf_name):

    print("pdf name in Sort_Prob is:",pdf_name)
    ...
    Calc_Final_Proba(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap,
                     X, y,  n_folds,pdf_name)

def Calc_Final_Proba(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap,X, y, n_folds, pdf_name
                     ,Calc_only_shap=False):

    run_name = str(pd.read_csv(final_folder + job_name + "_result_sorted.csv",index_col=0).index.values[0])

    print("pdf name in Calc_Final_Proba is:", pdf_name)

    with open(final_folder + "Rel_Feat_Names", 'rb') as fp:
        Rel_Feat_Names=pickle.load(fp)
    with open(final_folder + "cat_names", 'rb') as fp:
        cat_names=pickle.load(fp)
    ...
    cv_ind = 0
    skf = StratifiedKFold(n_splits=n_folds, random_state=0, shuffle=False)
    if ~isinstance(y, pd.DataFrame):
        y = pd.DataFrame(data=y,index=X.index)
    print('Start training of final model of: ', job_name, " at:", datetime.datetime.now())
    if Refit_Return_Model_Path!=None:
        if os.path.isdir(Refit_Return_Model_Path):
            lgbs = [f for f in os.listdir(Refit_Return_Model_Path) if f.endswith('_lgbm.txt')]
            sort_lgbs = [x for i in range(5) for x in lgbs if x[-10] == str(i)]
            print("Sorted_lgbs:", sort_lgbs)
        elif os.path.isfile(Refit_Return_Model_Path):
            lgbs = [Refit_Return_Model_Path]
            sort_lgbs = [Refit_Return_Model_Path]
    for train_index, test_index in skf.split(X.values, y.values.flatten()):
        params_bu = params_dict
        X_train = X.iloc[train_index, :]
        y_train = y.iloc[train_index]
        X_val = X.iloc[test_index, :]
        y_val = y.iloc[test_index]
        lgb_train = lgb.Dataset(X_train.values, label=y_train.values.flatten(), categorical_feature=cat_ind,feature_name=Rel_Feat_Names,
                                free_raw_data=False)
    ...
    # train
        ...
        if Refit_Return_Model_Path == None:
            ...
            #             valid_names=["Train"], feature_name=Rel_Feat_Names,
            #             evals_result=evals_result, verbose_eval=VERBOSE_EVAL)
        gbm.save_model(final_folder + "Results_" + job_name + "_" + run_name + str(cv_ind)+"_lgbm.txt")
        ...
        if not Calc_only_shap:
            ...
            test_ind_list.append(test_index)
            gbm.save_model(final_folder + job_name + run_name + "_cv_ind" + str(cv_ind) + '_lgbm.txt')
        if calc_shap:
            print("Calculating SHAP Values for:", job_name, " at:", datetime.datetime.now(),\
                " Fold Number:", str(cv_ind + 1), "out of:", str(n_folds))
            shap_values = gbm.predict(X_val.values, num_iteration=gbm.best_iteration, pred_contrib=True,num_threads=10)
            shap_values_tmp_df = pd.DataFrame(data=shap_values, index=test_index)
            shap_values_df = pd.concat([shap_values_df, shap_values_tmp_df], axis=0)
        cv_ind += 1
    # Add here the X_Test and Y_Test results
    print("Final training of ", job_name, " took %s seconds ---" % (time.time() - start_time))
    if ~Calc_only_shap:
        with open(final_folder + job_name + "_evals_result_list", 'wb') as fp:
                pickle.dump(evals_List, fp)
        with open(final_folder + job_name + "y_proba_list", 'wb') as fp:
                pickle.dump(y_proba_list, fp)
        y_proba_df.to_csv(final_folder + job_name + "_OnlyPROB.csv")
    with open(final_folder + job_name + "test_ind_list", 'wb') as fp:
            pickle.dump(test_ind_list, fp)
    if calc_shap:
        shap_values_df.to_csv(final_folder + job_name + "_shap_values.csv", index=False)

    print(('Finished predicting...', job_name, "at time:", datetime.datetime.now()))
    if ~Calc_only_shap:
        create_pdf_prob(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap,n_folds, y, X, pdf_name)


def create_pdf_prob(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, n_folds, y_test, X_test,
                    pdf_name):
    print("pdf name in create_pdf_prob is:",pdf_name, end=' ')
    run_name = str(pd.read_csv(filepath_or_buffer=final_folder + job_name + "_result_sorted.csv", index_col=0).iloc[0, 0])
    SHAP_PDF_PATH = final_folder + pdf_name + "_" + str(run_name) + ".pdf"
    pdf = PdfPages(SHAP_PDF_PATH)
    y_pred_val = []
    y_test_val = []
    ...
    y_proba_df = pd.read_csv(final_folder + job_name + "_OnlyPROB.csv",index_col=0)
    if y_proba_df.shape[1]>1:
        y_proba_df=y_proba_df.drop(y_proba_df.columns[0], axis=1)
    # with open(final_folder + job_name + "test_ind_list", 'rb') as fp:
    #     test_ind_list=pickle.dump(fp)
    #
    ...
    if calc_shap:
        if os.path.isfile(final_folder + job_name + "_shap_values.csv"):
            shap_values_df = pd.read_csv(final_folder + job_name + "_shap_values.csv")
            df = shap_values_df.values
        else:
            print("Calculating SHAP Values for:", job_name, " at:", datetime.datetime.now())
            Calc_Final_Proba(metric, Save_2_folder, final_folder, job_name, Target_ID, calc_shap, X_test, y_test
                             ,n_folds,pdf_name,Calc_only_shap=True)
        # print "plotting SHAP Values for:", job_name, " at:", datetime.datetime.now()
        # fig = plt.figure(figsize=(40, 30))
        df = pd.read_csv(final_folder + job_name + "_shap_values.csv")
        ...
        # else:
        shap.summary_plot(shap_values[:,:X_display.shape[1]], features=X_display, show=False, auto_size_plot=True, max_display=30)
        ax = plt.gca()
        ax.tick_params(axis="y",labelsize="20")
        plt.savefig(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png', bbox_inches='tight',dpi=DPI)
        plt.close("all")
        img = mpimg.imread(final_folder + "Results_" + job_name + "_" + run_name + 'SHAP.png')
        imgplot = plt.imshow(img)
        plt.axis('off')
